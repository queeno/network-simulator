...
public class Config {
	...
	throws NumberFormatException, Exception {
		...
		if(properties.containsKey("traffic_pattern")) {
			...
			case MAPREDUCE:
				getStrProperty(properties, "input_directory");
				getStrProperty(properties, "map_reduce_master");
				
				String str_master = properties.getProperty("map_reduce_master");
				
				if (str_master.equalsIgnoreCase("X"))
					setParam("map_reduce_master", String.valueOf((Mesh.DefaultMaster())));
				
				System.out.println("MapReduce master set successufully to " + mapReduceMaster() + "\n");
	...
	throws NumberFormatException, Exception {
		...
		getBoolProperty(properties, "run_forever");
	}
	...
	public static void setDefaultParams() {
		...
		setParam("trace_file",        "");
		setParam("input_directory",	  "");
		setParam("map_reduce_master", "0" );
	...
	private static void getBoolProperty(Properties properties, String key) {
		if(!m_params.containsKey(key))
			System.err.println("Error: missing key "+key);
		if(properties.containsKey(key))
			m_params.put(key, Boolean.parseBoolean(properties.getProperty(key)));
	}
	...
	private static boolean getBoolParam(String key) {
		return ((Boolean) m_params.get(key)).booleanValue();
	}
	...
	public static boolean	   runForever()		{ return getBoolParam("run_forever");   }
	...
	public static String	   mapReduceDirectory()	{ return getStrParam("input_directory"); }
	public static String	   mapReduceMaster(){ return getStrParam("map_reduce_master"); }
